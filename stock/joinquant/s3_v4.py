...
# ---- v4.4 现金管理：闲置资金买货币ETF(511880)，T+0随时变现加仓 ----
# bear空仓期/range留30%现金期/永久持仓预算闲置期，把闲余现金买货币ETF赚利息；
# 需要加仓时T+0卖出当日资金可用买股票。货币ETF几乎无波动、不参与策略止损止盈。
CASH_ETF_CODE = '511880.XSHG'    # 货币ETF(华宝添益)，T+0、几乎无波动、年化~2%
CASH_ETF_RESERVE = 10000         # 保留不投入货币ETF的现金缓冲(永久持仓预算+日常)
...
def _redeem_cash_etf(context, current_data):
    """v4.4 卖出全部货币ETF(511880)释放资金用于加仓。T+0当日资金可用买股票。"""
    if CASH_ETF_CODE not in context.portfolio.positions:
        return
    pos = context.portfolio.positions[CASH_ETF_CODE]
    amt = pos.closeable_amount
    if amt <= 0:
        return
    price = current_data[CASH_ETF_CODE].last_price if CASH_ETF_CODE in current_data else 0.0
    if price <= 0 or (isinstance(price, float) and np.isnan(price)):
        return
    order_amount_in_chunks(CASH_ETF_CODE, -amt, price)
    log.info("💵【现金管理】卖出货币ETF 511880 %d 份(≈%.0f元)释放资金" % (amt, amt * price))


def _invest_cash_etf(context):
    """v4.4 盘后把闲余现金买货币ETF(511880)，T+0随时可变现加仓。
    保留 CASH_ETF_RESERVE 现金缓冲；已持有则不追加(简化)。"""
    if CASH_ETF_CODE in context.portfolio.positions:
        return   # 已持有货币ETF，不再追加
    avail = context.portfolio.available_cash - CASH_ETF_RESERVE
    if avail < 10000:   # 货币ETF最小100份×~100元≈1万
        return
    try:
        bars = get_bars(CASH_ETF_CODE, count=1, unit='1d', fields=['close'], include_now=True)
    ...
    if price <= 0:
        return
    ordered = order_buy_once(CASH_ETF_CODE, avail, price)
    if ordered >= 100:
        log.info("💵【现金管理】盘后买入货币ETF 511880 %d 份(≈%.0f元)，T+0随时可变现"
...
def check_perm_hold(context, current_data):
    ...
    for cfg in todo:
        ...
        if price >= current_data[code].high_limit:
            continue   # 涨停不追
        _redeem_cash_etf(context, current_data)   # v4.4 变现货币ETF释放资金
...
def market_open(context):
    ...
    # v4.3 永久持仓检查（深度价值买入，与策略仓位独立；check_perm_hold内部按需变现货币ETF）
    check_perm_hold(context, current_data)

    # 5.1 开盘即跌停的持仓：立即挂跌停价限价单卖出（永久持仓/货币ETF跳过）
    for security in list(context.portfolio.positions.keys()):
        if security in g.perm_hold_done or security == CASH_ETF_CODE:
...
def _check_stop_loss(context, current_data, current_date):
    ...
    for security in list(current_positions.keys()):
        if security in g.perm_hold_done or security == CASH_ETF_CODE:
            continue   # v4.3 永久持仓 / v4.4 货币ETF 不参与止损
        if security in g.pending_exit_stocks:
            continue
        if g.trailing_stop_last_date.get(security) == current_date:
            continue
...
def _process_30m_signals(context, current_data, current_date):
    ...
    for security in list(context.portfolio.positions.keys()):
        if security in g.perm_hold_done or security == CASH_ETF_CODE:
            continue   # v4.3 永久持仓 / v4.4 货币ETF 不止盈
        if security in g.pending_exit_stocks:
            continue
        if g.trailing_stop_last_date.get(security) == current_date:
            continue
    ...
    if g.today_buy_count >= MAX_BUYS_PER_DAY:
        return
    # v4.4 加仓前先变现货币ETF释放资金
    _redeem_cash_etf(context, current_data)
...
def after_market_close(context):
    ...
    记录锁死价拖入补卖池，次日开盘继续死磕。"""
    # v4.4 盘后把闲余现金买货币ETF(511880)，T+0次日可变现加仓
    _invest_cash_etf(context)
